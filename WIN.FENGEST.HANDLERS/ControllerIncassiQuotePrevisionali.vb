...
Namespace FINANCIAL_ARTIFACTS
    Public Class ControllerIncassiQuotePrevisionali
        ...
        Public Sub New()
            Ps.BeginTransaction()
        End Sub
        Private Function Current() As IncassoQuotaPrevisionale
            Return DirectCast(MyBase.m_current, IncassoQuotaPrevisionale)
        End Function
...
#Region "Metodi di ricerca dal database"
        Protected Overridable Function GetIncassiForSoggetto(ByVal Soggetto As Azienda) As ArrayList
            ...
            Dim query As Query = ps.CreateNewQuery("MapperIncassoQuotaPrevisionale")
            Dim mainCriteria As CompositeCriteria = New CompositeCriteria(AbstractBoolCriteria.Operatore.AndOp)
            mainCriteria.AddCriteria(New AndExp(Criteria.Equal("TIPO_DOCUMENTO", "'IQP'"), Criteria.Equal("ID_AZIENDA", Soggetto.IdAzienda)))
...
#Region "Metodi CRUD"
        Protected Overrides Sub LoadAll()
            ...
            Dim ps As IPersistenceFacade = DataAccessServices.Instance.PersistenceFacade
            Dim query As Query = ps.CreateNewQuery("MapperIncassoQuotaPrevisionale")
            Dim mainCriteria As Criteria = Criteria.Equal("TIPO_DOCUMENTO", "'IQP'")
            query.AddWhereClause(mainCriteria)
        ...
        Public Overloads Sub CreateNew(ByVal DataRegistrazione As DateTime, ByVal DataDocumento As DateTime, _
            ...
            m_current = New IncassoQuotaPrevisionale(DataRegistrazione, DataDocumento, soggetto, competenza, Note, soggetto.Conto.Valuta, Importo, settore)
        ...
        Protected Overrides Sub CommitTransaction()
            Try
                ...
                For Each elem As ItemIncassoQuotaPrevisionale In listaPosizioni
                    Ps.MarkNew(elem)
                Next
                ''Aggiorno l'azienda
                ...
                '            Ps.MarkDirty(elem.Utente.GetActiveDelegaFor(elem.SoggettoEsecutore, elem.DataDocumento)) ', elem.Competenza.GetDataRange.Finish))
                '        End If
                '    End If
                'Next
        ...
        Public Overrides Function GetCategoryObject() As String
            Return "IncassoQuotaPrevisionale"
        End Function
