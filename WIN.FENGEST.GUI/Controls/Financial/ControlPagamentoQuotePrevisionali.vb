...
Public Class ControlPagamentoQuotePrevisionali
    Private controllerSettori As New ControllerSettori
    Private controller As New FINANCIAL_ARTIFACTS.ControllerIncassiQuotePrevisionali
...
#Region "Metodo per visualizzare le combobox, le informazioni di stato  e i dati del soggetto pagante"
    ...
    Public Overrides Sub Nested_NotifyParent()
        MasterControl.Instance.ParentForm.Text = "" & MyBase.State.StateName & " incasso quota previsionale"
        MasterControl.Instance.GetMdi.ToolStripStatusLabel1.Text = "" & MyBase.State.StateName & " incasso quota previsionale "
        Me.lblCurrentFunction.Text = UCase("" & MyBase.State.StateName & " incasso  quota   previsionale")
...
#Region "metodi per la navigazione"
    Protected Overrides Sub Navigate()
        Try
            ...
            ParameterList.Add("TipoDocumento", "IQP")
...
#Region "Metodi per la gestione  della griglia"
    ...
    Private Function PrepareGridLoading() As Boolean
        ...
        Dim gridLoader As New GridLoaderNewIQP(Me.GridPosizioni, controller)
...
#Region "Funzionalità sulle posizioni"
    ...
    Private Sub cmdAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdAdd.DoClik
        Try
            frm = New FrmItemContabile("Add", controller, "IQP")
            frm.ShowDialog()
            frm.Dispose()
            ...
            Else
                DocumentMediatorFactory.GetDocumentMediator("IQP").RefeshItemsGrid(Me, controller)
            End If
            CorrectImporto()
        Catch ex As Exception
            ErrorHandler.ShowError(ex)
