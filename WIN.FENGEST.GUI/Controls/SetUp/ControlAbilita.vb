...
<SecureContext()> _
Public Class ControlAbilita
   Private controllerCausali As New ControllerAbilita
   Public Sub New()
      InitializeComponent()
    ...
    Public Overrides Sub Nested_NotifyParent()
        MasterControl.Instance.ParentForm.Text = "" & MyBase.State.StateName & " skill"
        MasterControl.Instance.GetMdi.ToolStripStatusLabel1.Text = "" & MyBase.State.StateName & " skill "
        Me.lblCurrentFunction.Text = UCase("" & MyBase.State.StateName & "  skill")
...
#End Region
   ...
   End Sub
    ...
    <Secure(Area:="Skill", Alias:="Creazione", Macroarea:="Setup")> _
    Protected Overrides Sub Nested_CheckSecurityForCreation()
        SecurityManager.Instance.Check()
    End Sub

    <Secure(Area:="Skill", Alias:="Aggiornamento", Macroarea:="Setup")> _
    Protected Overrides Sub Nested_CheckSecurityForChanging()
        SecurityManager.Instance.Check()
    End Sub


    <Secure(Area:="Skill", Alias:="Cancellazione", Macroarea:="Setup")> _
   ...
   Private Sub CmdSel_DoClik(ByVal Sender As Object, ByVal e As System.EventArgs) Handles CmdSel.DoClik
      Try
         ...
         If lstCausali.SelectedIndex = -1 Then
            MsgBox("Non è stata selezionato alcuno skill per l'aggiornamento. Selezionare uno skill.", MsgBoxStyle.Information)
            Exit Sub
         End If
         controllerCausali.LoadByDescrizione(lstCausali.SelectedItem.ToString)
   ...
   Private Sub CustomBar1_DoEliminaClick(ByVal Sender As Object, ByVal e As System.EventArgs) Handles CustomBar1.DoEliminaClick
      Try
         ...
         If lstCausali.SelectedIndex = -1 Then
            MsgBox("Non è stata selezionata alcuno skill per l'eliminazione. Selezionare uno skill.", MsgBoxStyle.Information)
            Exit Sub
         End If
         If MsgBox("Lo skill sarà eliminato solamente se nel sistema non ci sono altri riferimenti. Sicuro di voler cancellare lo skill selezionato? ", MsgBoxStyle.YesNo + MsgBoxStyle.Question, "Eliminazione utente") = MsgBoxResult.Yes Then
            controllerCausali.LoadByDescrizione(lstCausali.SelectedItem.ToString)
   ...
   Private Sub CustomBar1_DoStampaClick(ByVal Sender As System.Object, ByVal e As System.EventArgs) Handles CustomBar1.DoStampaClick
      Try
         ...
         print.PrintListaAbilita(controllerCausali)
      Catch ex As Exception
         ErrorHandler.ShowError(ex)
      End Try
   End Sub
