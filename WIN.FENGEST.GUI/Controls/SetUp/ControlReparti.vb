...
#End Region
   ...
   Private Sub CmdSel_DoClik(ByVal Sender As Object, ByVal e As System.EventArgs) Handles CmdSel.DoClik
      Try
         ...
         If lstCausali.SelectedIndex = -1 Then
            MsgBox("Non è stato selezionato alcun reparto per l'aggiornamento. Selezionare un reparto.", MsgBoxStyle.Information)
            Exit Sub
         End If
   ...
   Private Sub CustomBar1_DoEliminaClick(ByVal Sender As Object, ByVal e As System.EventArgs) Handles CustomBar1.DoEliminaClick
      Try
         ...
         If lstCausali.SelectedIndex = -1 Then
            MsgBox("Non è stato selezionato alcun reparto per l'eliminazione. Selezionare un reparto.", MsgBoxStyle.Information)
            Exit Sub
         End If
         If MsgBox("Il reparto sarà eliminato solamente se nel sistema non ci sono altri riferimenti al reparto. Sicuro di voler cancellare il reparto selezionato? ", MsgBoxStyle.YesNo + MsgBoxStyle.Question, "Eliminazione utente") = MsgBoxResult.Yes Then
            controllerCausali.LoadByDescrizione(lstCausali.SelectedItem.ToString)
